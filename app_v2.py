...
# DMC 2.8 compatibility must load before constructing the layout.
...
from components.brand import ROSCA_BLUE, ROSCA_GREEN, ROSCA_NAVY, ROSCA_SLATE, brand_lockup
...
def parameter_panel() -> dmc.Component:
    return dmc.Stack([
        dmc.Group([
            dmc.Stack([dmc.Text("ROSCA", size="xs", c="blue", fw=900, style={"letterSpacing": ".14em"}), dmc.Title("Define", order=2)], gap=1),
            dmc.Badge("Preview solver", color="yellow", variant="light"),
...
def make_2d(data: dict, scheme: str = "dark", mode: str = "profile") -> go.Figure:
    fig = go.Figure()
    for label, x, y, color in [("Main", data["main_x"], data["main_y"], ROSCA_BLUE), ("Gate", data["gate_x"], data["gate_y"], "#94A3B8")]:
    ...
    if mode == "clearance":
        fig.add_annotation(x=.02, y=.98, xref="paper", yref="paper", text="CLEARANCE INSPECTION", showarrow=False, font={"size":12})
    elif mode == "deviation":
        fig.add_annotation(x=.02, y=.98, xref="paper", yref="paper", text="DEVIATION VIEW · PREVIEW DATA", showarrow=False, font={"size":12})
...
def make_3d(data: dict, scheme: str = "dark") -> go.Figure:
    ...
    for label, xs, ys, phase, color in [("Main", data["main_x"][:n], data["main_y"][:n], 0, ROSCA_BLUE), ("Gate", data["gate_x"][:n], data["gate_y"][:n], math.pi, "#64748B")]:
        ...
        for z in z_values:
            ...
            xx.append(np.asarray(xs)*c - np.asarray(ys)*s)
            yy.append(np.asarray(xs)*s + np.asarray(ys)*c)
            zz.append(np.full(n, z))
...
def workspace_view():
    return dmc.Grid([
        ...
        dmc.GridCol(dmc.Stack([
            dmc.Group([
                dmc.Stack([dmc.Title("Design workspace", order=2), dmc.Text("Define → validate → simulate → inspect → deliver", size="sm", c="dimmed")], gap=2),
                dmc.Group([dmc.Badge(id="stage-badge", children="Ready", color="gray"), dmc.Text(id="result-id", size="xs", c="dimmed")], gap="xs"),
            ], justify="space-between"),
            analysis_toolbar(),
...
def shell():
    return dmc.AppShell([
        dmc.AppShellHeader(dmc.Group([
            brand_lockup(34, show_descriptor=True),
            dmc.Group([
                action_button("open", button_id="open"),
                action_button("save", button_id="save"),
                dmc.ColorSchemeToggle(button="light"),
                action_button("help", button_id="help"),
            ], gap=2),
        ], h="100%", px="md", justify="space-between"), withBorder=True),
        dmc.AppShellNavbar(p=["md"], w=78, children=dmc.Stack([
            *[dmc.Tooltip(dmc.ActionIcon(icon(i,20), id=f"nav-{k}", size="lg", variant="subtle"), label=label, position="right") for k,label,i in [
                ("define","Define","tabler:adjustments"),("simulate","Simulate","tabler:player-play"),("inspect","Inspect","tabler:search"),("deliver","Deliver","tabler:file-export"),
            ]],
            dmc.Divider(my="sm", w=40),
            dmc.Tooltip(dmc.ActionIcon(icon("tabler:brand-abstract",20), id="nav-identity", size="lg", variant="light", color="blue"), label="Visual identity", position="right"),
        ], align="center")),
        dmc.AppShellMain(dmc.Container([
            ...
            dmc.Box(id="route-main", children=workspace_view()),
            dmc.Box(id="route-identity", children=identity_page(), style={"display":"none"}),
        ], fluid=True, p="md")),
...
@app.callback(
    ...
    [State(f"{id_}","value") for id_ in ["main-external","gate-external","main-pitch","clearance","leading-angle","trailing-angle","points"]],
    State("analysis-store","data"), prevent_initial_call=False,
)
...
def export_click(_, *values):
    ...
    if report.errors:
        return no_update
    result_id, data, result_report = calculate_preview(params)
    project = {"name":"ROSCA project","version":"2.0","parameters":{k:getattr(params,k) for k in FIELDS},"result_id":result_id}
    blob = build_project_zip(project, data, result_report, {})
    return dcc.send_bytes(blob, "rosca-project.zip")


@app.callback(
    Output("route-main","style"), Output("route-identity","style"),
    Input("nav-identity","n_clicks"), Input("url","pathname"),
    prevent_initial_call=False,
)
def route_update(_, pathname):
    show_identity = pathname == "/identity"
    return ({"display":"none"} if show_identity else {}), ({"display":"block"} if show_identity else {"display":"none"})


@app.callback(Output("url","pathname"), Input("nav-identity","n_clicks"), prevent_initial_call=True)
def open_identity(_):
    return "/identity"
